    ...
    for file in $1/*; do
        if test -f $file; then
            if [ "${file##*.}"x = ${suffix}x ]; then
                ...
                curl -v -u admin:admin -F -F "maven2.groupId=$groupId" -F "maven2.artifactId=$prefix" "maven2.version=$version" -F "maven2.asset1=@$file" -F "maven2.asset1.extension=$extension" -F "maven2.asset1.classifier="$classifier"" $nexus_url
            fi
        elif [ -d $file ]; then
            getdir $file
        fi
